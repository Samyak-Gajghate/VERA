...
def route_after_grading(state: GraphState) -> str:
    ...
    Decides: generate now, rewrite and try again, or web search fallback?

    Logic:
    - If enough relevant docs found → generate
    - If no relevant docs AND still have retries left (retry_count < MAX_RETRIES) → rewrite_query
    - If no relevant docs AND retries exhausted (retry_count >= MAX_RETRIES) → web_search
    """
    ...
    if len(relevant_docs) >= MIN_RELEVANT_DOCS:
        ...
        # Out of retries — trigger web search fallback
        return "web_search"
...
def build_graph(db):
    ...
    workflow.add_edge("web_search", "generate")       # web search results flow into generate
