...
# ── NODE 3b: Web search fallback ──────────────────────────────────────────────
def web_search_node(state: GraphState) -> dict:
    """
    Executes a web search fallback using Tavily when vector search yields no relevant documents.
    Tags all retrieved web results with source_type="web".
    """
    import uuid
    from .config import TOP_K_WEB_SEARCH, TAVILY_API_KEY
    ...
    try:
        ...
        if api_key and TavilySearchResults is not None:
            tool = TavilySearchResults(max_results=TOP_K_WEB_SEARCH)
            tavily_results = tool.invoke({"query": question})
        else:
            print("TAVILY_API_KEY not set. Skipping live web search call.")
    ...
    if isinstance(tavily_results, list):
        for res in tavily_results:
            ...
            url = res.get("url", "web-source") if isinstance(res, dict) else "web-source"
            if content:
                web_docs.append(GradedDocument(
                    ...
                    relevance_score=1.0,
                    grade="relevant",
                    grade_reason="Retrieved via Tavily web search fallback",
                    source_type="web"
                ))
    ...
    return {
        ...
        "audit_log": [make_audit_entry(
            step="web_search",
            message=f"Web search executed via Tavily ({len(web_docs)} web sources retrieved)",
            data={
                "query": question,
                "web_doc_count": len(web_docs),
                "sources": [d.source for d in web_docs]
            }
        )]
    }
...
def generate_node(state: GraphState) -> dict:
    """
    Generates the answer from relevant documents (or web search fallback results).
    ...
    needs to know which text came from which file/URL.
    """
    question = state.get("rewritten_question") or state["question"]
    ...
    if not relevant_docs:
        return {
            "generation": "I could not find relevant information in the provided document corpus or web search to answer this question. Please try rephrasing your question.",
            "audit_log": [make_audit_entry(
                step="generate",
                message="No relevant documents or web search results found — returned refusal guardrail response",
                data={"relevant_doc_count": 0}
            )]
        }

    # Build the context string with source attribution and source type
    context_parts = []
    for doc in relevant_docs:
        source_type = getattr(doc, "source_type", "document")
        context_parts.append(f"[Source: {doc.source} ({source_type})]\n{doc.content}")
    context = "\n\n---\n\n".join(context_parts)
